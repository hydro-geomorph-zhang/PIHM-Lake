...
void HydroSurfLateral(Model_Data MD, int i,int j,int inabr)
{
	...
	double		Height_orifice_before=0;
	
	//----------------Calculation----------------------//
	// if (i==66||i==1450)
	// {
		// printf("pheasan branch");
	// }
	if (inabr>=0) 
	{
	...
	else
	{
		...
		else if (MD->Ele[i].BC[j] == 4)//banks of lake
		{
        ...
        else if (MD->Ele[i].BC[j] == 5)//Lake outlets
        {
		...
		else if (MD->Ele[i].BC[j] == 6)//for weir and orifice boundary
        { 
            Distance = sqrt(pow(MD->Ele[i].edge[0] * MD->Ele[i].edge[1] * MD->Ele[i].edge[2] / (4 * MD->Ele[i].area), 2) - pow(MD->Ele[i].edge[j] / 2, 2));
            MD->Distance[i][j] = Distance;
			lake_level_adjust  = MD->LakeSurfElev[MD->Land_lake_index[i]-1] - MD->LakeInitialElev[MD->Land_lake_index[i]-1] + MD->LakeOutletDecline[MD->Land_lake_index[i]-1];
            ...
            if (Dif_Y_Surf<=0)
            {
				...
				MD->Spill_flow[i][j] = 2/3*0.62*sqrt(2*GRAV)*MD->Weir_length[MD->Land_lake_index[i]-1]*pow(fabs(Dif_Y_Surf),3/2);
				Height_orifice_before = MD->Gate_height[MD->Land_lake_index[i]-1] + lake_level_adjust;
				if (Height_orifice_before>MD->DummyY[i])
				{
					MD->Orifice_flow[i][j] = 0.61*MD->Orifice_height[MD->Land_lake_index[i]-1]*MD->Weir_length[MD->Land_lake_index[i]-1]*sqrt(2*GRAV*(Height_orifice_before-MD->DummyY[i]));
				}
				else
				{
					MD->Orifice_flow[i][j] = 0;
				}
				
				MD->FluxSurf[i][j] = - MD->Spill_flow[i][j]*UNIT_C - MD->Orifice_flow[i][j]*UNIT_C;
            }
            else
            {
                MD->Spill_flow[i][j] = 0;
