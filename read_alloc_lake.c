...
void read_alloc_lake(char *filename, LakeData LD, Model_Data DS)
{
    ...
    FILE *lakeriverinterest_file;	/* Pointer to .lakeriverinterest file*/
...
//  	printf("\n  1) reading %s.lakegeom ... ", filename);
    ...
    //LD->LakeBoundary =(int **)malloc((LD->NumLake)*sizeof(int));
  	...
  	{
		fscanf(lakegeom_file, "%s %d %lf %lf %lf %lf %lf" ,tempchar, &(LD->Lake[i].NumBankEle), &(LD->Lake[i].SurfArea),&(LD->Lake[i].OutletElevDecline),&(LD->Lake[i].Weir_length),&(LD->Lake[i].Orifice_height),&(LD->Lake[i].Gate_height));
