...
main(int argc, char *argv[])
{
	...
	FILE           *Ofile[30];	/* Output file     */
	char           *ofn[30];
    FILE           *Ofile_Lake[11];	/* Output file     */
    char           *ofn_Lake[11];
	...
	ofn_Lake[10] = (char *)malloc((strlen(filename)+1024)*sizeof(char));
    strcpy(ofn_Lake[10], directory);
    strcat(ofn_Lake[10], filename);
    Ofile_Lake[10]=fopen(strcat(ofn_Lake[10], ".lakeFlux"),"w");
...
//    Lakeboundary_char = (char *)malloc((strlen(filename)+1024)*sizeof(char));
//    strcpy(Lakeboundary_char, directory);
//    strcat(Lakeboundary_char, filename);
//    strcat(Lakeboundary_char, ".lakeboundele");
//    Lakeboundary = fopen(Lakeboundary_char, "w");
	...
	while (total_t < cData.FinishTime)
	{
		 ...
		 */
            ...
            if ((int) total_t%cData.surfDInt == 0)
			{
                ...
                for (k=0;k<mData->NumEle;k++)
                {
                    fprintf(init1,"%lf\t%lf\t%lf\t%lf\t%lf",mData->EleIS[k], mData->EleSnowCanopy[k]+mData->EleSnowGrnd[k], mData->DummyY[k],mData->DummyY[k+mData->NumEle],mData->DummyY[k+2*mData->NumEle]);
                    fprintf(init1,"\n");
                    
                    fprintf(init2,"%lf\t%lf\t%lf\t%lf\t%lf",mData->EleIS[k], mData->EleSnowCanopy[k]+mData->EleSnowGrnd[k], mData->DummyY[k],mData->DummyY[k+mData->NumEle],mData->DummyY[k+2*mData->NumEle]);
                    fprintf(init2,"\n");
    ...
    for(i=0;i<24;i++)fclose(Ofile[i]);
		
    for(i=0;i<24;i++)free(ofn[i]);
