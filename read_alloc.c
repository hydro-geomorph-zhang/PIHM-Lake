  ...
  //	printf("\n  7) reading %s.forc ... ", filename);
    ...
    free(fn[6]);
  	...
  	for(i=0; i<DS->NumPrep; i++)
  		{
    		fscanf(forc_file, "%s %d", DS->TSD_Prep[i].name, &DS->TSD_Prep[i].index);
			DS->TSD_Prep[i].length = 648644;
  	...
  	for(i=0; i<DS->NumTemp; i++)
  		{
    		fscanf(forc_file, "%s %d", DS->TSD_Temp[i].name, &DS->TSD_Temp[i].index);
			DS->TSD_Temp[i].length = 648644;
  	...
  	for(i=0; i<DS->NumHumidity; i++)
  		{
    		fscanf(forc_file, "%s %d", DS->TSD_Humidity[i].name, &DS->TSD_Humidity[i].index);
			DS->TSD_Humidity[i].length = 648644;
  	...
  	for(i=0; i<DS->NumWindVel; i++)
  		{
    		fscanf(forc_file, "%s %d", DS->TSD_WindVel[i].name, &DS->TSD_WindVel[i].index);
			DS->TSD_WindVel[i].length = 648644;
			DS->windH[i] = 2;
  	...
  	for(i=0; i<DS->NumRn; i++)
  		{
    		fscanf(forc_file, "%s %d", DS->TSD_Rn[i].name, &DS->TSD_Rn[i].index);
			DS->TSD_Rn[i].length = 648644;
  	...
  	for(i=0; i<DS->NumP; i++)
  		{
    		fscanf(forc_file, "%s %d", DS->TSD_Pressure[i].name, &DS->TSD_Pressure[i].index);
			DS->TSD_Pressure[i].length = 648644;
  	...
  	for(i=0; i<DS->NumLC; i++)
  		{
    		fscanf(forc_file, "%s %d", DS->TSD_LAI[i].name, &DS->TSD_LAI[i].index);
			DS->TSD_LAI[i].length = 910;
  	...
  	for(i=0; i<DS->NumLC; i++)
  		{
    		fscanf(forc_file, "%s %d", DS->TSD_RL[i].name, &DS->TSD_RL[i].index);
			DS->TSD_RL[i].length = 910;
  	...
  	for(i=0; i<DS->NumMeltF; i++)
  		{
    		fscanf(forc_file, "%s %d", DS->TSD_MeltF[i].name, &DS->TSD_MeltF[i].index);
			DS->TSD_MeltF[i].length = 910;
...
//  	printf("\n  8) reading %s.ibc  ... ", filename);  
 ...
 // 	printf("done.\n");
  ...
  //	printf("\n  9) reading %s.para ... ", filename); 
	...
	fscanf(para_file, "%s %d", tempchar, &(CS->mean_output));
...
//	printf("\n  10) reading %s.centroid ... ", filename);
        fn[9] = (char *)malloc((strlen(filename)+1024)*sizeof(char));
        strcpy(fn[9], filename);
        centroid_elev = fopen(strcat(fn[9], ".centroid"), "r");
		free(fn[9]);
        ...
        else
        {
        	...
        	for (i=0;i<DS->NumEle;i++)
        	{
        		fscanf(centroid_elev,"%s %s	%lf	%lf", tempchar,tempchar,&DS->Ele[i].zmax, &DS->Ele[i].zmin);
        	}
