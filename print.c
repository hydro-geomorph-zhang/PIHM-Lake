...
void
avgResults_NV(FILE * fpin, double * tmpVarCal, Model_Data tmpDS, int tmpIntv, int tmpNumObj, double tmpt, int tmpInitObj, int mean_output)
{
	int             j;
	int             TmpIntv;
...
void
avgResults_NV_lake(FILE * fpin, double * tmpVarCal, int tmpIntv, int tmpNumObj, double tmpt, double * StateVariable, int mean_output)
{
    int             j;
    int             TmpIntv;
...
avgResults_MD(FILE * fpin, double * tmpVarCal, Model_Data tmpDS, int tmpIntv, int tmpNumObj, double tmpt, int tmpFC)
{
	...
	case 17:
	 ...
	 case 24:
            for(j=0;j<tmpNumObj;j++)
            {
                tmpVarCal[j]=tmpVarCal[j]+(tmpDS->Orifice_flow[j][0]);
            }
            break;
	 case 25:
            for(j=0;j<tmpNumObj;j++)
            {
                tmpVarCal[j]=tmpVarCal[j]+(tmpDS->Orifice_flow[j][1]);
            }
            break;
	 case 26:
            for(j=0;j<tmpNumObj;j++)
            {
                tmpVarCal[j]=tmpVarCal[j]+(tmpDS->Orifice_flow[j][2]);
            }
            break;
	 case 27:
            for(j=0;j<tmpNumObj;j++)
            {
                tmpVarCal[j]=tmpVarCal[j]+(tmpDS->Spill_flow[j][0]);
            }
            break;
	 case 28:
            for(j=0;j<tmpNumObj;j++)
            {
                tmpVarCal[j]=tmpVarCal[j]+(tmpDS->Spill_flow[j][1]);
            }
            break;
	 case 29:
            for(j=0;j<tmpNumObj;j++)
            {
                tmpVarCal[j]=tmpVarCal[j]+(tmpDS->Spill_flow[j][2]);
            }
            break;
	default:
		break;
...
PrintData(FILE ** outp, FILE ** outp_Lake, Control_Data * cD, Model_Data DS, LakeData LD, double t)
{
	if (cD->gwDInt >0)
	{
		avgResults_NV(outp[0], DS->PrintVar[0], DS, cD->gwDInt, DS->NumEle, t, 2 * DS->NumEle,cD->mean_output); //GW
	}
	
	if (cD->surfDInt >0) 
	{
		avgResults_NV(outp[1], DS->PrintVar[1], DS, cD->surfDInt, DS->NumEle, t, 0 * DS->NumEle,cD->mean_output); //Surf
	}
    if (cD->etInt >0)
    {
    ...
    if (cD->usDInt >0)
    {
        avgResults_NV(outp[7], DS->PrintVar[7], DS, cD->usDInt, DS->NumEle, t, 1 * DS->NumEle,cD->mean_output); //Unsat
    }
	if (cD->RechInt >0) 
	{
	...
	if (cD->surfDInt >0) 
	{
		...
		avgResults_MD(outp[24], DS->PrintVar[24], DS, cD->surfDInt, DS->NumEle, t, 24); //Orifice1
		avgResults_MD(outp[25], DS->PrintVar[25], DS, cD->surfDInt, DS->NumEle, t, 25); //Orifice2
		avgResults_MD(outp[26], DS->PrintVar[26], DS, cD->surfDInt, DS->NumEle, t, 26); //Orifice3
		avgResults_MD(outp[27], DS->PrintVar[27], DS, cD->surfDInt, DS->NumEle, t, 27); //Spill1
		avgResults_MD(outp[28], DS->PrintVar[28], DS, cD->surfDInt, DS->NumEle, t, 28); //Spill2
		avgResults_MD(outp[29], DS->PrintVar[29], DS, cD->surfDInt, DS->NumEle, t, 29); //Spill3
	}
	
    if (cD->surfDInt >0)
    {
        avgResults_NV_lake(outp_Lake[0], LD->PrintVar[0], cD->surfDInt, LD->NumLake, t, LD->LakeSurfDepth,cD->mean_output);  //LakeSurf
        avgResults_NV_lake(outp_Lake[1], LD->PrintVar[1], cD->gwDInt, LD->NumLake, t, LD->LakeGW,cD->mean_output);  //LakeGW
        ...
        // avgResults_MD_Lake(outp_Lake[10], LD->PrintVar[10], DS, LD, cD->surfDInt, LD->NumLake, t, 10); //Orifice
    }
}
...
/********* .lakesoil**********/
    ...
    /********* .riv**********/
    temp_string = (char *) malloc(1024 * sizeof(char));
    sprintf(temp_string,"%s.riv",filename);
    orig_file = fopen(temp_string,"r");
    free(temp_string);
    temp_string = (char *) malloc(1024 * sizeof(char));
    sprintf(temp_string,"%sinput/%s.riv",directory,filename);
    desti_file = fopen(temp_string, "w");
    fseek(orig_file, 0L, SEEK_END); // file pointer at end of file
    pos = ftell(orig_file);
    fseek(orig_file, 0L, SEEK_SET); // file pointer set at start
